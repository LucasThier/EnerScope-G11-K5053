...
| `organization.service.OrganizationServiceTest` | Unit | 8 |
| `organization.controller.OrganizationControllerTest` | Web | 7 |
| **Total** | | **59** |
...
Organization creation, member addition (with role/permission derivation) and
project addition.
...
| `addProjectPersistsAndLinksToOrganization` | Adding a project persists it linked to the organization and appends it to `Organization.projects`. |
| `addProjectRejectsUnknownOrganization` | An unknown organization id throws `IllegalArgumentException`; the project is never saved. |
...
| `addProjectReturnsCreatedProject` | `POST /organizations/{id}/projects` with a valid body → `201` with the created project's `name`/`description`. |
| `addProjectRejectsBlankFieldsWithValidationError` | Blank `name`/`description` → `400` `Validation error`; the service is never called. |
